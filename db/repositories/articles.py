...
# All valid entity column names — used to guard against SQL injection in
# update_entities() when building the SET clause dynamically.
_VALID_ENTITY_COLUMNS: frozenset[str] = frozenset({
    "entity_person",
    "entity_norp",
    "entity_fac",
    "entity_org",
...
class ArticleRepository:
    ...
    def update_entities(self, article_id: int, entity_data: dict) -> None:
        ...
        filtered = {
            col: val
            for col, val in entity_data.items()
            if col in _VALID_ENTITY_COLUMNS
        }

        if not filtered:
            log.debug(
                "update_entities: no valid entity columns for article_id=%d — skipping.",
                article_id,
            )
            return
        ...
        with self._conn._tx() as cur:
            cur.execute(
                f"UPDATE articles SET {set_clause} WHERE id = %(_article_id)s",
                params,
            )

        log.debug(
            "update_entities: wrote %d column(s) for article_id=%d.",
            len(filtered),
            article_id,
        )
    ...
    def update_state(self, article_id: int, state: str) -> None:
        ...
        with self._conn._tx() as cur:
            cur.execute(
                "UPDATE articles SET state = %s WHERE id = %s",
                (state, article_id),
            )
